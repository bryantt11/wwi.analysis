...
# Review data structure and summary statistics
str(sales.df)
summary(sales.df)
...
corr.sales.new<- corr.sales

# Write new files for corrplot 
saveRDS(corr.sales.new, "corr.sales.rds")
...
model.sales.new <- model.sales
str(model.sales.new)

# Write new files model sales dataset
saveRDS(model.sales.new, "model.sales.rds")
...
# Write new files model sales = normalized dataset
saveRDS(model.sales.df.norm, "model.sales.norm.rds")
...
norm.values <- preProcess(train.nn, method=c("center", "scale"))
train.norm.nn <- predict(norm.values, train.nn)
valid.norm.nn <- predict(norm.values, valid.nn)

# Write new files for train/test sets - 
saveRDS(train.norm.nn, "train.norm.nn.rds")
saveRDS(valid.norm.nn, "valid.norm.nn.rds")
...
nn.sales1 <- nnet(Profit~Sale.Key+
                        ...
                        Quantity+
                        Tax.Rate+
                        Calc.Unit.Cost, train.norm.nn, size=5, linout=T)
summary(nn.sales1)
...
nn.sales2 <- nnet(Profit~Sale.Key+
                   ...
                   Quantity+
                   Tax.Rate+
                   Calc.Unit.Cost, valid.norm.nn, size=5, linout=T)
summary(nn.sales2)
plotnet(nn.sales2, bias = FALSE)

# RMSE - error on training set
sales.pred1 <- predict(nn.sales1, train.norm.nn)
rmse1 <- sqrt(mean(sales.pred1^2))
print(rmse1)

# RMSE - error on training set
sales.pred2 <- predict(nn.sales2, valid.norm.nn)
rmse2 <- sqrt(mean(sales.pred2^2))
print(rmse2)

#---------------------KNN---------------------

model.sales.df4 <- dummy.data.frame(select(model.sales, -c(2,3,4,5,6,10,12,13,14,
                                                         15,16,17,18)))
model.sales.df4$Is.Order.Finalized.True <- ifelse(model.sales.df4$Is.Order.Finalized == "TRUE", 1, 0) 
model.sales.df4$Is.Order.Finalized.False <- ifelse(model.sales.df4$Is.Order.Finalized == "FALSE", 1, 0) 

model.sales.df4 <- select(model.sales.df4, -c(1))

# partition the dataset into 60% training and 40% validation sets 
set.seed(111)
train.index <- train.index <- sample(1:nrow(model.sales.df4), 0.6 * nrow(model.sales.df4))
train.knn <- model.sales.df4[train.index, ]
valid.knn <- model.sales.df4[-train.index, ]

# use preProcess() from the caret package to normalize
norm.values <- preProcess(train.knn, method=c("center", "scale"))
train.norm.df <- predict(norm.values, train.knn)
valid.norm.df <- predict(norm.values, valid.knn)

# Write new files for train/test sets - 
saveRDS(train.norm.df, "train.norm.df.rds")
saveRDS(valid.norm.df, "valid.norm.df.rds")
...
for(i in 1:14){
  knn.pred <- knn(train.norm.df, valid.norm.df,
                  cl = train.norm.df[, "SupplierLitware"], k = i)
  accuracy.df[i, 2] <- confusionMatrix(knn.pred, 
                                       factor(valid.norm.df[, "SupplierLitware"]))$overall[1]
}
accuracy.df
plot(accuracy.df)
...
KNN.CM <- confusionMatrix(knn.pred, factor(valid.norm.df[, "SupplierLitware"]))
KNN.CM
